			...
			public function get_providers() {
				return apply_filters( 'wc_shipment_tracking_get_providers', array(
					...
					'Czech Republic' => array(
						...
						'Česká pošta'
							=> 'https://www.postaonline.cz/trackandtrace/-/zasilka/cislo?parcelNumbers=%1$s',
						'DHL.cz'
							=> 'http://www.dhl.cz/cs/express/sledovani_zasilek.html?AWB=%1$s',
						'DPD.cz'
							=> 'https://tracking.dpd.de/parcelstatus?locale=cs_CZ&query=%1$s',
					),
					'Finland' => array(
						'Itella'
					...
					'United Kingdom' => array(
						...
						'InterLink'
							=> 'http://www.interlinkexpress.com/apps/tracking/?reference=%1$s&postcode=%2$s#results',
						'ParcelForce'
				...
				if ( $tracking_provider ) {
					...
					if ( $link_format ) {
						...
						if ( $for_email ) {
							...
							$tracking_link = sprintf( '<a href="%s" target="_blank">' . __( 'Click here to track your shipment', 'wc_shipment_tracking' ) . '.</a>', $link, $link );
						}
					}
					...
					if ( $custom_tracking_link ) {
						$tracking_link = sprintf( '<a href="%s" target="_blank">' . __( 'Click here to track your shipment', 'wc_shipment_tracking' ) . '.</a>', $custom_tracking_link );
					} elseif ( strstr( $tracking_number, '<a' ) ) {
						$tracking_link = sprintf( '<a href="%s" target="_blank">%s.</a>', $tracking_number, $tracking_number );
