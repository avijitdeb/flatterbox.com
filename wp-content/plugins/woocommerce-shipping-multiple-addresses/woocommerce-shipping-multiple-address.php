...
if ( is_woocommerce_active() ) {
    ...
    class WC_Ship_Multiple {
        ...
        public static function settings_scripts() {
            ...
            if ( isset($_GET['page']) && $_GET['page'] == 'wc-ship-multiple-products' ) {
        ...
        function draw_form() {
            ...
            if (!isset($_GET['order_id']) || empty($_GET['order_id'])) {
                ...
                if ( isset($_REQUEST['duplicate-form']) ) {
                    ...
                    require 'templates/duplicate-form.php';

                } elseif ( empty($addresses) || isset($_REQUEST['address-form']) ) {

                    require 'templates/address-form.php';
                    ...
                    if (! empty($contents)) {
                        ...
                        require 'templates/shipping-address-table.php';
        ...
        function display_meta_box($post) {
            ...
            jQuery(".pkg_tip_<?php echo $x .'_'. $pId .'_'. $i; ?>").tipTip({
                content: "<?php echo str_replace( '"', '\"', trim($tip)); ?>"
            });
                <?php
                    endforeach;
                endforeach;
        ...
        public function calculate_taxes( $cart = null, $packages = null ) {
            ...
            foreach ( $packages as $idx => $package ) {
                ...
                foreach ( $package['contents'] as $cart_item_key => $values ) {
                     ...
                     */
                        ...
                        $taxes                 = $cart->tax->calc_tax( $line_price, $item_tax_rates );
                        $line_subtotal_tax     = array_sum( $taxes );
                ...
                foreach ( $package['contents'] as $cart_item_key => $values ) {
                    ...
                    if ( $cart->prices_include_tax ) {
                        ...
                        $discounted_price      = $cart->get_discounted_price( $values, $base_price, true );
                        $discounted_taxes      = $cart->tax->calc_tax( $discounted_price * $values['quantity'], $item_tax_rates );
            ...
            if ( $cart->round_at_subtotal ) {
                $cart->tax_total          = $cart->tax->get_tax_total( $cart_taxes );
                $cart->taxes              = array_map( array( $cart->tax, 'round' ), $cart_taxes );
            } else {
