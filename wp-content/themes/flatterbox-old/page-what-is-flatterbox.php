<?php
/* Template Name: FB What is a Flatterbox */
get_header(); ?>
		...
		<div class="largebox whatisfb">
			<div class="boximage-holder colmatch">
				...
				if (strlen(get_field('main_image')) > 0) : 
					...
					$mainimage = home_url().'/wp-content/uploads/2015/07/what-is-flatterbox-birthday.jpg';
				endif; 
				?>
					...
					if (strlen(get_field('secondary_image')) > 0) :
						...
						if ( ! empty( $occasions ) && ! is_wp_error( $occasions ) ) {
						    ...
						    foreach ( $occasions as $occ ) {
						        ...
						        $n = 7 - $count;
						        $term_list .= '<a href=http://www.flatterbox.com/choose-card/><div class="occasion'.$active.'" style="z-index: '.$n.'; background-image: url(\'' . z_taxonomy_image_url( $occ->term_id ) . '\');"><div class="occasionname"><span>' . $occ->name . '</span></div></div></a>';
						    	$count++;
