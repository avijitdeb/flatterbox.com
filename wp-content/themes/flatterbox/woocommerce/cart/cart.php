				...
				<div class="product-info teaser">
					...
					<div class="product-description">
						...
						<div class="info-row">
							...
							<?php 
								$curency_sym = '';
								$total_final_price = WC()->cart->get_product_price( $_product );
								/* Removed to hide update pricing in line and place as a line item

								$total_final_price = str_replace('<span class="amount">', '', str_replace('</span>', '', WC()->cart->get_product_price( $_product )));
								//echo '!'.intval(substr($total_final_price, 5)).'!';
								$curency_sym = substr($total_final_price, 0, 5);
								if ($_SESSION["add10"] == 1) : $total_final_price = intval(substr($total_final_price, 5)) + 4.99; endif;
								*/
								echo apply_filters( 'woocommerce_cart_item_price', $curency_sym.$total_final_price, $cart_item, $cart_item_key );
							?>
							<?php
							if($_GET["modified"] == 1) 
							{
						...
						<div class="info-row">
							...
							<?php 
								$curency_sym = '';
								$total_final_price_sub = WC()->cart->get_product_subtotal( $_product, $cart_item['quantity'] ); 
								/* Removed to hide update pricing in line and place as a line item
								$total_final_price_sub = str_replace('<span class="amount">', '', str_replace('</span>', '', WC()->cart->get_product_subtotal( $_product, $cart_item['quantity'] )));
								//echo '!'.intval(substr($total_final_price_sub, 5)).'!';
								$curency_sym = substr($total_final_price_sub, 0, 5);
								if ($_SESSION["add10"] == 1) : $total_final_price_sub = intval(substr($total_final_price_sub, 5)) + 4.99; endif;
								*/
								echo apply_filters( 'woocommerce_cart_item_subtotal', $curency_sym.$total_final_price_sub, $cart_item, $cart_item_key );
							?>
						</div>
