				...
				<div class="invite-actions">
					<?php if (strlen(get_field('unique_url', $_SESSION["sentimentPID"])) > 0 ) : ?>
						<?php if ( check_for_mobile() ) : // Is Mobile ?>
							...
							<div><pre><?php echo home_url().'/?fb='.get_field('unique_url', $_SESSION["sentimentPID"]); ?></pre></div>
						<?php else : ?>
							<p>Share your Flatterbox with Flatterers using your unique link : 
								<input type="text" readonly='readonly' name="unique-url" id="unique-url" value="<?php echo home_url().'/?fb='.get_field('unique_url', $_SESSION["sentimentPID"]); ?>">
							</p>
						<?php endif; ?>
					<?php endif; ?>
				</div>
